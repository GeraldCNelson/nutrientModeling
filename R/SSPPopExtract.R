...
#' @source \url{https://tntcat.iiasa.ac.at/SspDb/download/common/SspDb_country_data_2013-06-12.csv.zip}
...
scenarios <-
  unique(dt.SSP.regions$scenario) #There are 21 scenarios; 4 each for dt.SSP.regions1, 2, 3, and 5 and 5 for dt.SSP.regions 4.
...
dt.SSP.regions3.pop.tot.IIASA <-
  dt.SSP.regions3[variable == "Population", c("region_code", keepYearList), with = FALSE]
...
dt.SSP.regions3.pop.IIASA$ageGenderCode <-
  gsub("Aged", "", dt.SSP.regions3.pop.IIASA$ageGenderCode)
dt.SSP.regions3.pop.IIASA$ageGenderCode[dt.SSP.regions3.pop.IIASA$gender == "Female"] <-
  paste("dt.SSP.regionsF",
        dt.SSP.regions3.pop.IIASA$ageGenderCode[dt.SSP.regions3.pop.IIASA$gender == "Female"],
        sep = "")
dt.SSP.regions3.pop.IIASA$ageGenderCode[dt.SSP.regions3.pop.IIASA$gender == "Male"] <-
  paste("dt.SSP.regionsM",
        dt.SSP.regions3.pop.IIASA$ageGenderCode[dt.SSP.regions3.pop.IIASA$gender == "Male"],
        sep = "")
dt.SSP.regions3.pop.IIASA$ageGenderCode <-
  gsub("-", "_", dt.SSP.regions3.pop.IIASA$ageGenderCode)
dt.SSP.regions3.pop.IIASA$ageGenderCode <-
  gsub("\\+", "Plus", dt.SSP.regions3.pop.IIASA$ageGenderCode)
dt.SSP.regions3.pop.IIASA <-
...
ageRowsToSum <- c(
  "dt.SSP.regionsF15_19",
  "dt.SSP.regionsF20_24",
  "dt.SSP.regionsF25_29",
  "dt.SSP.regionsF30_34",
  "dt.SSP.regionsF35_39",
  "dt.SSP.regionsF40_44",
  "dt.SSP.regionsF45_49"
)

#pull out the relevant rows
dt.SSP.regions3.pop.F15_49.IIASA <-
  dt.SSP.regions3.pop.IIASA[dt.SSP.regions3.pop.IIASA$ageGenderCode %in% ageRowsToSum, c("region", "ageGenderCode", yearList)]
...
dt.SSP.regions3.pop.F15_49.sum.IIASA <-
  as.data.frame(dt.tmp[, lapply(.SD, sum), by = region, .SDcols = yearList])
#add age column with the single value for the new age group
dt.SSP.regions3.pop.F15_49.sum.IIASA$ageGenderCode <-
  rep("dt.SSP.regionsF15_49",
      nrow(dt.SSP.regions3.pop.F15_49.sum.IIASA))
#get rid of now extraneous rows in dt.SSP.regions3.pop.IIASA so no double counting
dt.SSP.regions3.pop.IIASA <-
  dt.SSP.regions3.pop.IIASA[!dt.SSP.regions3.pop.IIASA$ageGenderCode %in% ageRowsToSum, ]
#add the dt.SSP.regionsF15_49 row to dt.SSP.regions3.pop.IIASA
dt.SSP.regions3.pop.IIASA <-
  rbind(dt.SSP.regions3.pop.IIASA,
        dt.SSP.regions3.pop.F15_49.sum.IIASA)
#sort by region
dt.SSP.regions3.pop.IIASA <-
  dt.SSP.regions3.pop.IIASA[order(dt.SSP.regions3.pop.IIASA$region), ]
...
pop.temp <-
  as.data.frame(dt.tmp[, lapply(.SD, sum), by = region, .SDcols = yearList])
temp <-
  pop.temp$X2010 - dt.SSP.regions3.pop.tot.IIASA$X2010 #this is a dumb thing to do. At least make sure they are sorted by region
...
kids.0_4 <-
  dt.SSP.regions3.pop.IIASA[dt.SSP.regions3.pop.IIASA$ageGenderCode %in% kidsRows, c("region", yearList)]
...
temp.nonPL <-
  dt.SSP.regions3.pop.F15_49.sum.IIASA[match(temp.preg$region,
                                             dt.SSP.regions3.pop.F15_49.sum.IIASA$region), cols] -
  temp.preg[, cols] -
...
temp.kids <-
  cbind(dt.kids.sum,
        ageGenderCode = "dt.SSP.regionsKids0_4",
        stringsAsFactors = FALSE)
temp.preg <-
  cbind(temp.preg,
        ageGenderCode = "dt.SSP.regionsPreg",
        stringsAsFactors = FALSE)
temp.lact <-
  cbind(temp.lact,
        ageGenderCode = "dt.SSP.regionsLact",
        stringsAsFactors = FALSE)
temp.nonPL <-
  cbind(temp.nonPL,
        ageGenderCode = "dt.SSP.regionsF15_49",
        stringsAsFactors = FALSE)

#add new rows to dt.SSP.regions3.pop.IIASA
dt.SSP.regions3.pop.IIASA <-
  rbind(dt.SSP.regions3.pop.IIASA, temp.preg, temp.lact, temp.nonPL)
dt.SSP.regions3.pop.IIASA <-
  dt.SSP.regions3.pop.IIASA[with(dt.SSP.regions3.pop.IIASA, order(region, ageGenderCode)),]

#check to see if population totals are the same. Uncomment to test
dt.SSP.regions3.pop.tot.IIASA <-
  dt.SSP.regions3.pop.tot.IIASA[order(dt.SSP.regions3.pop.tot.IIASA$region), ]
dt.tmp <- data.table(dt.SSP.regions3.pop.IIASA)
setkey(dt.tmp, "region")
pop.temp <-
  as.data.frame(dt.tmp[, lapply(.SD, sum), by = region, .SDcols = yearList])
...
dt.pop.melt <-
  melt(
    ...
    id.vars = c("region", "ageGenderCode"),
    measure.vars = yearList,
    value.name = "pop.value"
  )
...
repCons <- function(nutReq, common.nut) {
  ...
  dt.temp[, (paste(common.nut, "sum", sep = ".")) := lapply(.SD, sum), by =
            c("region", "year"), .SDcols = c(reqlist)]
  #remove the .prod columns
  dt.temp[, c(reqlist) := NULL]
  dt.temp.sum <-
    unique(dt.temp[, c("region", "year", paste(common.nut, "sum", sep = ".")), with =
                     F])

  dt.pop.IIASA <- as.data.table(dt.SSP.regions3.pop.tot.IIASA)
  dt.pop.IIASA.melt <- melt(
    dt.pop.IIASA,
    id.vars = "region",
    measure.vars = yearList,
    variable.name = "year",
  ...
  dt.temp[, (paste(common.nut, "fin", sep = ".")) := lapply(.SD, function(x)
    x / dt.temp$pop.tot), .SDcols = c(paste(common.nut, "sum", sep = "."))]
  dt.temp[, c("pop.tot", paste(common.nut, "sum", sep = ".")) := NULL]
  dt.temp.melt <- melt(
    dt.temp,
    id.vars = c("region", "year"),
    measure.vars = paste(common.nut, "fin", sep = "."),
    variable.name = "nutrient",
    variable.factor = FALSE,
    value.name = "value"
  )
  return(dcast.data.table(dt.temp.melt, region + nutrient ~ year, value.var =
                            "value"))
...
creationInfo <-
  rbind(creationInfo,
        paste("dt.SSP.regions data:", dt.SSP.regionsdataZipFileName))
...
for (i in 1:length(reqs)) {
  ...
  dt.temp.internal <-
    repCons(eval(parse(text = nutReq)), eval(parse(text = common[i])))
