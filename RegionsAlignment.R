...
regions.IMPACT3.plus <- rbind(regions.IMPACT3.plus,
                              data.frame(region_code =CTY,region_name = regionname,region_members = lst,stringsAsFactors=FALSE))
...
regions.IMPACT115$mappingCode_2 <- substring(regions.IMPACT115$mappingCode_2,2,nchar(regions.IMPACT115$mappingCode_2)-1)
...
temp3 <- gsub(" ","",temp3[,1:2])
temp4 <- as.data.frame(cbind(temp3[,1],temp2$cty),stringsAsFactors = FALSE)
...
# SSP regions
regions.SSP <- as.data.frame(sort(unique(SSP$country_code)),stringsAsFactors = FALSE) #there are 194 regions
...
for (i in 1:nrow(dt.SSP)){
  rnum <- rnumFromGrep(dt.SSP$country_code[i],regions.IMPACT3$region_members)
...
dt.SSP.IMPACT3 <- dt.SSP[,lapply(.SD, sum), by=c("model","scenario","variable","region_code","region_name"), 
                         .SDcols=keepYearList]
...
rownames(regions.SSP) <- rownames(regions.IMPACT115) <- rownames(regions.ISO) <- 
...
save("regions.IMPACT115", "regions.IMPACT115.plus", "regions.IMPACT3", "regions.IMPACT3.plus", 
     "regions.ISO","regions.SSP","dt.SSP.IMPACT3" , file = "data/regions.RData")
