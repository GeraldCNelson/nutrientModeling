...
# Contributors to the work include Brendan Power (for coding assistance), Joanne E. Arsenault 
# (for constructing the nutrient requirements worksheet) and  
...
common.EAR <- intersect(sort(colnames(nutrients)),sort(colnames(req.EAR)))
common.RDA.vits <- intersect(colnames(nutrients),colnames(req.RDA.vits))
common.RDA.minrls <- intersect(colnames(nutrients),colnames(req.RDA.minrls))
common.RDA.macro <- intersect(colnames(nutrients),colnames(req.RDA.macro))
common.UL.vits <- intersect(colnames(nutrients),colnames(req.UL.vits))
common.UL.minrls <- intersect(colnames(nutrients),colnames(req.UL.minrls))
common.AMDR <- intersect(colnames(nutrients),colnames(req.AMDR)) # this is empty right now
...
#20% of the population for 0-4 year olds (this is the number of 4 year olds) 
#plus 80% of the population of 5-9 year olds (this is number of 5-8 year olds) – 
#and then adjust all the others in a similar manner.  

male.ssp <- c("SSPM5_9","SSPM10_14","SSPM15_19", "SSPM20_24", "SSPM25_29", "SSPM30_34", "SSPM35_39", 
              "SSPM40_44", "SSPM45_49", "SSPM50_54", "SSPM55_59", "SSPM60_64", 
              "SSPM65_69", "SSPM70_74", "SSPM75_79", "SSPM80_84", "SSPM85_89", 
              "SSPM90_94", "SSPM95_99", "SSPM100Plus")
male.dri <- c("Chil4_8","M9_13","M14_18", "M19_30", "M19_30", "M31_50", "M31_50", "M31_50", "M31_50", 
              "M31_50", "M51_70", "M51_70", "M51_70", "M70Plus", "M70Plus", "M70Plus", 
              "M70Plus", "M70Plus", "M70Plus", "M70Plus")
...
for (j in reqs) {
  ...
  chldrn.male.SSP <- chldrn.male[chldrn.male$ageGenderCode == "Inf0_0.5",2:length(temp)] * 1/8 + 
    chldrn.male[chldrn.male$ageGenderCode == "Inf0.5_1",2:length(chldrn.male)] * 1/8 + 
    chldrn.male[chldrn.male$ageGenderCode == "Chil1_3",2:length(chldrn.male)] * 6/8
  ...
  preg.potent <- temp2[temp2$ageGenderCode %in% c("F14_18", "F19_30", "F31_50"),]
  preg.potent <- preg.potent[preg.potent$ageGenderCode == "F14_18",2:length(preg.potent)] /3 + 
    preg.potent[preg.potent$ageGenderCode == "F19_30",2:length(preg.potent)] /3 + 
    preg.potent[preg.potent$ageGenderCode == "F31_50",2:length(preg.potent)] /3
  preg.potent$ageGenderCode <- "SSPF15_49"
  
  lact <- temp2[temp2$ageGenderCode %in% c("Lact14_18", "Lact19_30", "Lact31_50"),]
  lact <- lact[lact$ageGenderCode == "Lact14_18",2:length(lact)] /3 + 
  ...
  preg <- temp2[temp2$ageGenderCode %in% c("Preg14_18", "Preg19_30", "Preg31_50"),]
  preg <- preg[preg$ageGenderCode == "Preg14_18",2:length(preg)] /3 + 
  ...
  temp2 <- temp2[(!temp2$ageGenderCode %in% male[,2] & 
                    ...
                    !temp2$ageGenderCode %in% c("Lact14_18", "Lact19_30", "Lact31_50")),]
