...
# this file is generated by IMPACTdataLoading.R. Only needs to be redone when new IMPACT data arrives
dt.df1 <- readRDS(file="data/IMPACTdatatable.rds") 

#keep only every fifth year
# keep every fifth year, code to define keepYearList is now in setup.R
#keepYearList <- c("X2010", "X2015", "X2020", "X2025", "X2030", "X2035", "X2040", "X2045", "X2050")
dt.df1<- dt.df1[year %in% keepYearList,]

# read in nutrients data ----
# rds file generated in nutrientdataloading.R
nutrients <- readRDS(file = "results/nutrients_final.rds")
dt.nutrients <- as.data.table(nutrients)

#replaced this source file with readin of results from rds files
#generated in SSPPopExtract.R
req.EAR <- readRDS("results/req.EAR.percap.rds" )
req.RDA.vits <- readRDS("results/req.RDA.vits.percap.rds")
req.RDA.minrls <- readRDS("results/req.RDA.minrls.percap.rds")
req.RDA.macro <- readRDS("results/req.RDA.macro.percap.rds")
req.UL.vits <- readRDS("results/req.UL.vits.percap.rds")
req.UL.minrls <- readRDS("results/req.UL.minrls.percap.rds")
...
dt.budget <- unique(dt.df1[,c(key(dt.df1),"budget.Pw","budget.Pc","budget.Pcon","pcGDP"),with=F])
...
reqList <- c("req.EAR","req.RDA.vits" ,"req.RDA.minrls", "req.RDA.macro", "req.UL.vits", "req.UL.minrls")

for (i in 1:length(reqList)) {
  #get list of nutrients from the requirement df
  nut.list <- unique(eval(parse(text = paste(reqList[i],"$nutrient",sep=""))))
  #nuts.reduced <- nutrients[, (names(nutrients) %in% nut.list)]
  #keep only nutrients in the requirements being worked on
  dt.nuts.reduced <- dt.nutrients[,c("IMPACT_code",nut.list,"staple.code","food.group.code"), with = FALSE]
  ...
  temp2 <- dt.d3[,(nut.list.Q) := lapply(.SD,function(x) (x * dt.d3[['food']] * 10)), .SDcols = nut.list]
  temp3 <- temp2[,(nut.list.Q.sum) := lapply(.SD, sum), .SDcols = nut.list.Q, by = key(dt.d3)]
  dt.food.group.sum <- unique(temp3[,c("scenario","region","food.group.code","year",nut.list.Q.sum),with=F])
  
  # get sum of nutrient intake by staple
  setkey(dt.d3,"scenario","region","staple.code","year")
  temp3 <- temp2[,(nut.list.Q.sum) := lapply(.SD, sum), .SDcols = nut.list.Q, by = key(dt.d3)]
  dt.staples.sum <- unique(temp3[,c("scenario","region","staple.code","year",nut.list.Q.sum),with=F])
  
  # get sum of nutrient intake by country
  setkey(dt.d3,"scenario","region","year")
  temp3 <- temp2[,(nut.list.Q.sum) := lapply(.SD, sum), .SDcols = nut.list.Q, by = key(dt.d3)]
  dt.food.sum <- unique(temp3[,c("scenario","region","year",nut.list.Q.sum),with=F])
  
  # convert the requirements dt from wide (years as columns) to long (add year and value columns)
  dt.reqs.long <- melt(eval(parse(text = reqList[i])), 
  ...
  dt.reqs <- dcast.data.table(dt.reqs.long, region + year ~ nutrient,value.var = "nut_req")
  ...
  for (j in 1:length(nut.list)){
    ...
    dt.food.tot[,(nut.list.ratio[j]):= eval(parse(text = nut.list[j]))/eval(parse(text = nut.list.Q.sum[j]))]
  }
  
  dt.food.ratio <- dt.food.tot[,nut.list.ratio, with=FALSE]
  colMax <- function(data) lapply(data, max, na.rm = TRUE)
  colMin <- function(data) lapply(data, min, na.rm = TRUE)
  ...
  # 
  saveRDS(dt.food.tot, file = paste("results/",reqList[i],".results.",Sys.Date(),".rds",sep=""))
  
  #reshape the results to get years in columns
  dt.food.tot.long <- melt(dt.food.tot, 
  ...
  dt.food.tot.wide <- dcast.data.table(dt.food.tot.long, scenario + region + nutrient ~ year,value.var = "nut_req")
  ...
  writeData(wbGeneral, eval(parse(text = reqList[i])), sheet=reqList[i], startRow=1, startCol=1, rowNames = FALSE)
  addStyle(wbGeneral, sheet=reqList[i], style=numStyle, rows = 1:nrow(eval(parse(text = reqList[i])))+1, 
           cols=2:ncol(eval(parse(text = reqList[i]))), gridExpand = TRUE)
  setColWidths(wbGeneral, sheet=reqList[i], cols = 1:ncol(eval(parse(text = reqList[i]))), widths="auto")
  wbInfoGeneral[(nrow(wbInfoGeneral)+1),] <- c(reqList[i],paste("metadata on nutrients included in ",reqList[i]))
  ...
  for (j in 1:length(nut.list)){
    ...
    writeData(wbGeneral, temp, sheet=nut.list[j], startRow=1, startCol=1, rowNames = FALSE,withFilter = TRUE)
    setColWidths(wbGeneral, sheet=nut.list[j], cols = 1, widths="auto", ignoreMergedCells = FALSE)
    addStyle(wbGeneral, sheet=nut.list[j], style=numStyle, rows = 1:nrow(temp), 
             cols=2:ncol(temp), gridExpand = TRUE)
    ...
    temp <- dt.food.tot.long[nutrient %like% "ratio" & nutrient %like% nutrient.name & scenario %in% scenario.name,]
    temp$year<- gsub("X","",temp$year)
    temp$year <- as.numeric(temp$year)
    gg <- ggplot(data=temp,
                 aes(x = year, y=nut_req,  color = region)) + geom_line()+ 
      theme(
        axis.text.x= element_text(color="black",angle=0, size=9, vjust=0.5),
        axis.text.y= element_text(color="black", size=9, vjust=0.5),
        axis.title.y = element_text(color="black",size=9, vjust=0.5),
        legend.text = element_text(color="black",size=7, vjust=0.5),
        plot.title = element_text(color="black",face="bold",size=11, hjust=0.5,vjust=1),
      ...
      ggtitle(paste("Cons. share of require., ", nutrient.name,", ",unique(temp$scenario),sep="")) +
 ...
 print(plotSheet)
    ...
    insertPlot(wbGeneral, sheet = plotSheet, width = 9, height = 5, fileType = "png", units = "in", startRow = 5)
    wbInfoGeneral[(nrow(wbInfoGeneral)+1),] <- c(nut.list[j], paste("Ratio results for ",nutrient.name))
    ...
    maxRowNum <- which(dt.food.ratio[,eval(parse(text = colnames(cMax)[j]))] == cMax[1,eval(parse(text = colnames(cMax)[j]))])
    minRowNum <- which(dt.food.ratio[,eval(parse(text = colnames(cMin)[j]))] == cMin[1,eval(parse(text = colnames(cMin)[j]))])
    writeData(wbGeneral, dt.food.tot[maxRowNum,], sheet=plotSheet, startRow=1, startCol=2, rowNames = FALSE,withFilter = FALSE)
    writeData(wbGeneral, paste("Country and year with max value for the ",nutrient.name, " ratio", sep=""), sheet=plotSheet, startRow=1, startCol=1, rowNames = FALSE,withFilter = FALSE)
    writeData(wbGeneral, dt.food.tot[minRowNum,], sheet=plotSheet, startRow=3, startCol=2, rowNames = FALSE,withFilter = FALSE)
    writeData(wbGeneral, paste("row with min value for the ",nutrient.name, " ratio", sep=""), sheet=plotSheet, startRow=3, startCol=1, rowNames = FALSE,withFilter = FALSE)
    addStyle(wbGeneral, sheet=plotSheet, style=numStyle, rows = 1:4, 
             cols=2:ncol(dt.food.tot), gridExpand = TRUE) 
    setColWidths(wbGeneral, sheet=plotSheet, cols = 1:ncol(dt.food.tot[minRowNum,]), widths="auto", ignoreMergedCells = FALSE)
    wbInfoGeneral[(nrow(wbInfoGeneral)+1),] <- c(plotSheet,paste("Max/min rows and graph of results for ",nutrient.name))
